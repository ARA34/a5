...
class Body(tk.Frame):
    ...
    def get_text_entry(self) -> str:
        return self.message_editor.get('1.0',
                                       'end').rstrip()
...
class NewContactDialog(tk.simpledialog.Dialog):
    ...
    def body(self, frame):
        """
        Creates body of newcontactdialog
        """
...
class MainApp(tk.Frame):
    ...
    def add_contact(self):
        ...
        if self.direct_messenger is None:
            ...
            contact = tk.simpledialog.askstring("Input",
                                                "Enter the name" +
                                                " of a profile: ")
    ...
    def recipient_selected(self, recipient):
        ...
        new_msgs = [self.profile.new_messages[0].get_recipient(),
                    self.profile.new_messages[0].get_message()]
        # print(f"nm: {new_msgs}")
    ...
    def configure_server(self):
        """
        Prompts the user to enter details to
        configure(change) server, username, and password.
        """
        if self.direct_messenger is None:
            print("WARNING: You must create or load a profile first")
            return
            ...
            self.profile.save_profile(
                fh.get_profile_path(self.direct_messenger.username))

    def publish(self, message: str):
        """
        Posts user's message on server.
        """
        if self.direct_messenger is not None:
            dsp.post(dsm_object=self.direct_messenger,
                     message=message)
        else:
            pass
    ...
    def check_new(self):
        ...
        if self.loaded is True:
            ...
            if continue_check:
                dm_list = dsm_var.retrieve_new()
                # print(f"General Message: {dm_list}")
                if len(dm_list) >= 1:
                    for dm in dm_list:
                        self.body.insert_contact_message(dm.get_recipient()
                                                         + ": "
                                                         + dm.get_message())
                ...
                self.body.insert_contact_message("WARNING: You must " +
                                                 "create or load a " +
                                                 "profile first")
        else:
            pass
    ...
    def open_file(self) -> None:
        """
        Opens a file and reads the contents of the file.
        Loads an exisitng profile on local device to server.
        """
        ...
        if Path(file_path).suffix == ".dsu":
            ...
            if (self.server is not None and self.username is not None and
                self.password is not None and self.direct_messenger is None):
    ...
    def new_profile(self) -> None:
        """
        Creates new profile and connects to
        ICS32 distributed social website
        """
        ud1 = NewContactDialog(self.root,
                               title="Creating ICS32 Distributed Account")
        s_prof = Profile(dsuserver=ud1.server,
                         username=ud1.user,
                         password=ud1.pwd)
