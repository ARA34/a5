...
class DsuProfileError(Exception):
    ...
    handler that you should catch in your own code. It
...
class Post(dict):
    """
    The Post class is responsible
    for working with individual user posts. It currently
...
class Profile:
    ...
    """
    add_post accepts a Post object
    as parameter and appends it to the posts list. Posts
    are stored in a list object in
    the order they are added. So if multiple Posts objects
    are created, but added to the Profile in
    a different order, it is possible for the
    list to not be sorted by the
    ...
    To determine which post to delete
    you must implement your own search operation on
    ...
    """
    save_profile accepts an existing dsu file to save
    the current instance of Profile
    to the file system.
    ...
    def set_new_messages(self):
        ...
        ds_messenger to retrieve new messages.
        """
        dsm = DirectMessenger(dsuserver=self.dsuserver,
    ...
    def set_all_messages(self):
        """
        Uses the function from
        ds_messenger to retrieve all messages.
        """
        dsm = DirectMessenger(dsuserver=self.dsuserver,
                              username=self.username,
                              password=self.password)
        if dsm.join():
            ...
            for i in self.all_messages:
                new_list.append([i.get_recipient,
                                 i.get_message])
    ...
    """
    load_profile will populate the current instance
    of Profile with data stored in a
    DSU file.
